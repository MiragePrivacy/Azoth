...
/// Represents an EVM opcode with its byte value and immediate data size.
/// Covers opcodes needed for CFG construction, encoding, and planned transforms
...
impl Opcode {
    /// Parses a raw byte into an Opcode and the number of immediate bytes to follow.
    ...
    /// Returns true if the opcode is relevant for control flow graph construction.
    ...
    /// Returns the byte representation of the opcode.
